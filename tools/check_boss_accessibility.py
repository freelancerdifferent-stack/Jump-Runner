...
if game:
    ...
    require("constbaselead=105+approach*185" in game and "constlead=baselead+boss.recoil" in game and "boss.x=player.x+lead" in game, "Sentinel must cycle through reachable attack passes while preserving visible hit recoil")
    ...
    require("boss.coreopen=boss.intro<=0&&boss.hitcd<=0&&!boss.passspent&&baselead<150&&boss.recoil<=0" in game, "Sentinel core must open during a reachable unspent close pass after recoil settles")
    require("if(baselead>=165)boss.passspent=false" in game, "Sentinel must rearm only after the underlying pass visibly retreats")
...
print("boss_reachable_arena=yes two_button_kill_path=yes visible_boss_onboarding=yes safe_area_boss_cue=yes one_hit_per_pass=yes hit_recoil_aware=yes missed_window_teaching=yes boss_core_hit_status=yes boss_phase_status=yes boss_intro_status=yes boss_defeat_status=yes atomic=yes polite=yes integrity_remaining=yes")

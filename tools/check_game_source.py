...
if HTML.is_file():
    ...
    require('passspent:false' in flat and 'if(lead>=165)boss.passspent=false' in flat and '!boss.passspent&&lead<150' in flat and 'boss.passspent=true' in flat, "Sentinel must allow at most one core hit per approach pass and rearm only after retreat")
    require('constattacklaney=340+math.sin(boss.t*4.3)*18' in flat and 'constlanemix=math.max(0,math.min(1,(175-lead)/25))' in flat and 'boss.y=patroly+(attacklaney-patroly)*lanemix' in flat, "Sentinel core-open passes must converge into a reachable Dash/Stomp attack lane")

...
if text:
    ...
    require("functionconfirmskipfirstrunguide()" in text, "first-run guide skip must use a confirmation step")
    require("skip.textcontent='skipguide'" in text and "skip.onclick=confirmskipfirstrunguide" in text, "first-run menu must route the skip control through confirmation")
    require("skip.setattribute('aria-label','reviewskipguidedfirstrun')" in text, "guide skip control must explain that it opens a review step")
    require("panel.setattribute('role','dialog')" in text and "panel.setattribute('aria-modal','true')" in text and "panel.setattribute('aria-labelledby','skipguidetitle')" in text, "skip confirmation must expose accessible dialog semantics")
    require("theguideteachesjump,dash,flow,checkpoints,andthesentinel" in text and "youcanreplayitlaterfromhome" in text, "skip confirmation must explain what is being skipped and how to recover it")
    require("keep.textcontent" not in text or "keepguide" in text, "skip confirmation must provide a keep-guide path")
    require("keep.onclick=()=>{clearskipguidedialog();showmenu();}" in text, "keep-guide action must return without persisting completion")
    require("confirm.onclick=()=>{clearskipguidedialog();skipfirstrunguide();}" in text, "skip-anyway action must be the only confirmation path to persistence")
    require("keep.focus();" in text, "skip confirmation must focus the safe keep-guide action by default")
    ...
    require("saveonboardingdone(true);showmenu();" in text, "confirmed guide skip must persist completion and rebuild the normal menu")

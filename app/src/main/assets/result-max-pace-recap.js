...
(()=>{
 ...
 function decorateResult(){
  ...
  const note=document.createElement('span');note.className='result-max-pace-note';note.textContent=record?'NEW RECORD':'BEST '+best.toFixed(1)+'s';
  card.setAttribute('aria-label',record?`New maximum pace hold record: ${held.toFixed(1)} seconds.`:`Maximum pace held for ${held.toFixed(1)} seconds this run. Personal best ${best.toFixed(1)} seconds.`);

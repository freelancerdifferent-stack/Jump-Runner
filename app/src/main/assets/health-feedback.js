...
(()=>{
 ...
 let previous=health,announceTimer=0,criticalAnnounced=false;
 ...
 function announceRecovery(){announce('Integrity restored. '+health+' of '+maxHealth+' remaining.');}
 function announceCritical(){announce('Critical integrity. '+health+' of '+maxHealth+' remaining.');}
 const base=renderHealth;renderHealth=function(){const before=previous;base();if(health<before){pulse('integrity-hit');if(health===1&&!criticalAnnounced){criticalAnnounced=true;announceCritical();}}else if(health>before&&state==='play'){pulse('integrity-restore');if(health>1)criticalAnnounced=false;announceRecovery();}previous=health;};
 const reset=resetRun;resetRun=function(){previous=maxHealth;criticalAnnounced=false;status.textContent='';clearTimeout(announceTimer);reset();};
})();

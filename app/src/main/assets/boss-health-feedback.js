...
(()=>{
 ...
 let previous=boss.hp,announceTimer=0;
 function announce(){clearTimeout(announceTimer);status.textContent='';announceTimer=setTimeout(()=>{status.textContent='Sentinel core hit. '+boss.hp+' of '+boss.maxHp+' integrity remaining.';},20);}
 function refresh(){if(boss.hp<previous&&!boss.dead){note.textContent='CORE HIT · '+boss.hp+'/'+boss.maxHp;note.classList.remove('show');void note.offsetWidth;note.classList.add('show');announce();}previous=boss.hp;}
 const base=updateBoss;updateBoss=function(dt){base(dt);refresh();};const reset=resetBoss;resetBoss=function(){reset();previous=boss.maxHp;status.textContent='';clearTimeout(announceTimer);note.classList.remove('show');};
})();

'use strict';
// Crystal pickup feedback: compact HUD pulse and combo acknowledgement without changing scoring.
(()=>{
  ...
  let previous=crystals;
  function pulse(){
    target.classList.remove('crystal-pulse');void target.offsetWidth;target.classList.add('crystal-pulse');
    const r=target.getBoundingClientRect(),note=document.createElement('div');note.className='crystal-float';note.textContent=flow>=4?'FLOW + CRYSTAL':'+ CRYSTAL';note.style.left=(r.left+r.width/2)+'px';note.style.top=(r.bottom+4)+'px';document.body.appendChild(note);setTimeout(()=>note.remove(),560);
  }
  const base=updateHud;
  updateHud=function(){base();if(state==='play'&&crystals>previous)pulse();previous=crystals;};
  const reset=resetRun;resetRun=function(){previous=0;reset();};
})();

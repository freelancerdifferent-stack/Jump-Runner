...
(()=>{
  ...
  Object.assign(cue.style,{
    ...
    opacity:'0',transition:'opacity .18s ease, transform .18s ease, padding .18s ease',zIndex:'18',
    whiteSpace:'nowrap',maxWidth:'calc(100vw - 32px)',overflow:'hidden',textOverflow:'ellipsis'
  });
  document.body.appendChild(cue);
  const EXPLAIN_MS=2400;
  let shown=false,announced=false,lockedSince=0,compact=false;
  function setExpanded(){
    compact=false;
    cue.textContent='ARENA MODE · AUTO-RUN PAUSED · JUMP / DASH TO FIGHT';
    cue.style.padding='8px 14px';
    cue.style.letterSpacing='.1em';
  }
  function setCompact(){
    compact=true;
    cue.textContent='ARENA MODE · JUMP / DASH';
    cue.style.padding='6px 11px';
    cue.style.letterSpacing='.08em';
  }
  function refresh(now){
    ...
    if(locked!==shown){
      ...
      if(locked){lockedSince=now||performance.now();setExpanded();}
      else{lockedSince=0;setExpanded();}
    }
    if(locked&&!compact&&(now||performance.now())-lockedSince>=EXPLAIN_MS)setCompact();
    if(locked&&!announced){

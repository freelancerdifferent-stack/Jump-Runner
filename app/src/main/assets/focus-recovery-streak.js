...
(()=>{
 ...
 let streak=readNumber(STORAGE_KEY),best=readNumber(BEST_STORAGE_KEY),attempted=false,lastCleared=false,lastLabel='',newBest=false,introTimer=0,liveTimer=0;
 const style=document.createElement('style');
 style.textContent='.focus-recovery-streak{margin:8px 0 2px;padding:9px 12px;border:1px solid #74f7c530;border-radius:14px;background:#08150f99;text-align:left}.focus-recovery-streak.is-reset{border-color:#ff8b9a30;background:#17090d88}.focus-recovery-streak.is-best{box-shadow:0 0 0 1px #ffd86b30 inset}.focus-recovery-streak.is-milestone{border-color:#ffd86b55;background:#17140899;box-shadow:0 0 20px #ffd86b12}.focus-recovery-streak .kicker{color:#74f7c5;font:900 9px/1.2 system-ui;letter-spacing:.14em}.focus-recovery-streak.is-reset .kicker{color:#ff9aab}.focus-recovery-streak.is-best .kicker,.focus-recovery-streak.is-milestone .kicker{color:#ffd86b}.focus-recovery-streak .target{margin-top:4px;color:#fff;font:900 12px/1.3 system-ui}.focus-recovery-streak .detail{margin-top:3px;color:#bcd7df;font:700 9px/1.35 system-ui}.focus-best-menu{margin:8px auto 2px;width:max-content;max-width:100%;padding:6px 11px;border:1px solid #ffd86b2c;border-radius:999px;background:#17140899;color:#ffe9a6;font:900 9px/1.2 system-ui;letter-spacing:.11em;text-transform:uppercase}.focus-best-menu.is-active{border-color:#74f7c542;background:#08150fb3;color:#9dffe0;box-shadow:0 0 18px #74f7c512}.focus-best-menu.is-milestone{border-color:#ffd86b55;color:#ffe9a6;box-shadow:0 0 20px #ffd86b14}.focus-run-momentum,.focus-live-milestone{position:fixed;z-index:22;left:50%;top:max(84px,calc(env(safe-area-inset-top) + 72px));transform:translate(-50%,-8px);opacity:0;pointer-events:none;min-width:220px;max-width:min(72vw,360px);padding:8px 13px;border:1px solid #74f7c542;border-radius:999px;background:#07120ebd;color:#9dffe0;text-align:center;font:900 10px/1.25 system-ui;letter-spacing:.1em;text-transform:uppercase;box-shadow:0 8px 26px #0007;transition:opacity .18s ease,transform .18s ease}.focus-run-momentum.show,.focus-live-milestone.show{opacity:1;transform:translate(-50%,0)}.focus-run-momentum.is-milestone,.focus-live-milestone{border-color:#ffd86b66;background:#171408d9;color:#ffe9a6}.focus-run-momentum.is-close{border-color:#ffd86b66;box-shadow:0 8px 26px #0007,0 0 20px #ffd86b12}.focus-live-milestone{z-index:28;top:max(112px,calc(env(safe-area-inset-top) + 98px));box-shadow:0 8px 26px #0008,0 0 24px #ffd86b18}.focus-run-momentum .sub,.focus-live-milestone .sub{display:block;margin-top:2px;color:#bcd7df;font:700 8px/1.2 system-ui;letter-spacing:.04em;text-transform:none}@media(max-height:390px){.focus-recovery-streak{margin-top:6px;padding:7px 10px}.focus-recovery-streak .target{font-size:10px}.focus-recovery-streak .detail{font-size:8px}.focus-best-menu{margin-top:5px;padding:5px 9px;font-size:8px}.focus-run-momentum{top:max(58px,calc(env(safe-area-inset-top) + 48px));padding:6px 10px;font-size:9px}.focus-live-milestone{top:max(82px,calc(env(safe-area-inset-top) + 70px));padding:6px 10px;font-size:9px}}@media(prefers-reduced-motion:reduce){.focus-run-momentum,.focus-live-milestone{transition:none;transform:translate(-50%,0)}}';
 ...
 function clearsToNext(value){const next=nextMilestone(value);return next?Math.max(0,next.value-value):0}
 ...
 function clearLiveMilestone(){
  ...
  const card=document.querySelector('.focus-live-milestone');if(card)card.remove();
 }
 function showRunMomentum(){
  ...
  const milestone=milestoneAt(streak),next=nextMilestone(streak),remaining=clearsToNext(streak),card=document.createElement('div');
  card.className='focus-run-momentum'+(milestone?' is-milestone':'')+(remaining===1?' is-close':'');card.setAttribute('role','status');card.setAttribute('aria-live','polite');card.setAttribute('aria-atomic','true');
  const headline=milestone?`${milestone.label} · STREAK ×${streak}`:(remaining===1&&next?`ONE CLEAR TO ${next.label}`:`FOCUS MOMENTUM ×${streak}`);
  const sub=next?`Keep it alive · next milestone ×${next.value} · ${remaining} ${remaining===1?'clear':'clears'} away`:`Best ×${best} · keep the streak alive`;
 ...
 function showLiveMilestone(milestone){
  clearLiveMilestone();if(!milestone||state!=='play')return;
  const next=nextMilestone(streak),card=document.createElement('div');
  card.className='focus-live-milestone';card.setAttribute('role','status');card.setAttribute('aria-live','polite');card.setAttribute('aria-atomic','true');
  card.innerHTML=`${milestone.label} · FOCUS ×${streak}<span class="sub">${next?`Momentum secured · next milestone ×${next.value}`:'Top focus milestone secured'}</span>`;
  document.body.appendChild(card);requestAnimationFrame(()=>card.classList.add('show'));
  liveTimer=setTimeout(()=>{card.classList.remove('show');liveTimer=setTimeout(()=>{card.remove();liveTimer=0;},220);},1500);
 }
 function onFocusResolved(event){
  ...
  if(lastCleared){const milestone=milestoneAt(streak);if(milestone)showLiveMilestone(milestone);}
 }

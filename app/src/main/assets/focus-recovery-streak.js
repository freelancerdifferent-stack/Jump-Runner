...
(()=>{
 ...
 function renderResult(){
  ...
  const card=document.createElement('div');card.className='focus-recovery-streak'+(lastCleared?'':' is-reset')+(newBest?' is-best':'');card.setAttribute('role','status');card.setAttribute('aria-live','polite');card.setAttribute('aria-atomic','true');
  const kicker=document.createElement('div');kicker.className='kicker';kicker.textContent=newBest?'NEW BEST MOMENTUM':(lastCleared?'COACHING MOMENTUM':'COACHING RESET');
 ...
 function renderMenuBest(){
  ...
  const badge=document.createElement('div');badge.className='focus-best-menu'+(streak>0?' is-active':'');
  if(streak>0){
   badge.textContent=`FOCUS MOMENTUM ×${streak} · BEST ×${best}`;
   badge.setAttribute('aria-label',`Current focus recovery streak ${streak}. Best ${best}.`);
  }else{

...
(()=>{
 ...
 let announced=false,hideTimer=0;
 ...
 function check(){
   ...
   if(pct>=.88){announced=true;clearTimeout(hideTimer);hideCue();void cue.offsetWidth;cue.setAttribute('aria-hidden','false');cue.classList.add('show');hideTimer=setTimeout(hideCue,760);}
 }
 const hud=updateHud;updateHud=function(){hud();check();};
 const reset=resetRun;resetRun=function(){announced=false;clearTimeout(hideTimer);hideTimer=0;hideCue();reset();};
})();

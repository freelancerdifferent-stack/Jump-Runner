'use strict';
// Finish approach cue: communicates the final stretch without touching completion logic.
(()=>{
 ...
 let announced=false;
 function check(){if(state!=='play'||announced)return;const pct=player.x/LEVEL_END;if(pct>=.88){announced=true;cue.classList.remove('show');void cue.offsetWidth;cue.classList.add('show');}}
 const hud=updateHud;updateHud=function(){hud();check();};const reset=resetRun;resetRun=function(){announced=false;cue.classList.remove('show');reset();};
})();

...
// and its victory guidance has cleared, so end-of-run messages never compete for attention.
(()=>{
 ...
 function setProgressEmphasis(active){if(progressTrack)progressTrack.classList.toggle('finish-near',Boolean(active));}
 function clearCue(){clearTimeout(hideTimer);hideTimer=0;pendingAfterPause=false;hideCue();setProgressEmphasis(false);}
 function playArrivalFeedback(){if(typeof chord==='function')chord([659,784],.075,.018,'sine');if(typeof haptic==='function')haptic(12);}
 function showCue(withFeedback=true){clearTimeout(hideTimer);hideCue();void cue.offsetWidth;cue.setAttribute('aria-hidden','false');cue.classList.add('show');setProgressEmphasis(true);if(withFeedback)playArrivalFeedback();hideTimer=setTimeout(()=>{hideTimer=0;hideCue();},760);}
 function check(){

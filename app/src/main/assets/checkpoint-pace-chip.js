...
function clearTransitionCue(){if(transitionTimer){clearTimeout(transitionTimer);transitionTimer=0;}if(streakTimer){clearTimeout(streakTimer);streakTimer=0;}paceChip.classList.remove('recovered','slipped','streak');}
...
function renderPace(detail){
 ...
 const bestStreakActive=isBest&&bestStreak>=2;
 ...
 paceChip.setAttribute('aria-label',recovered?(isBest?`${label}. Pace recovered with a new best split.`:`${label}. Pace recovered. ${Math.abs(delta).toFixed(1)} seconds ahead of best split.`):(slipped?`${label}. Pace slipped. ${delta.toFixed(1)} seconds behind best split.`:(bestStreakActive?`${label}. ${bestStreak} best splits in a row.`:(isBest?`${label}. New best split.`:`${label}. ${Math.abs(delta).toFixed(1)} seconds ${delta<0?'ahead of':'behind'} best split.`))));

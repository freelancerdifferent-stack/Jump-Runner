...
paceStyle.textContent='.checkpoint-pace-chip{--pace-meter:0;position:fixed;z-index:6;left:50%;top:max(104px,calc(env(safe-area-inset-top) + 86px));transform:translateX(-50%);min-width:176px;padding:5px 11px 7px;border:1px solid #69edff33;border-radius:14px;background:#06121ecc;box-shadow:0 8px 20px #0004;color:#bdefff;font:850 9px/1.2 system-ui;letter-spacing:.1em;text-align:center;pointer-events:none;white-space:nowrap}.checkpoint-pace-chip.ahead{border-color:#74f7c555;color:#74f7c5}.checkpoint-pace-chip.behind{border-color:#ffd86b55;color:#ffd86b}.checkpoint-pace-chip.best{border-color:#8fffe077;color:#8fffe0;box-shadow:0 8px 20px #0004,0 0 15px #74f7c51f}.checkpoint-pace-chip.recovered{animation:pace-recovered .42s ease-out;box-shadow:0 8px 20px #0004,0 0 18px #74f7c52e}.checkpoint-pace-chip.boss{top:max(112px,calc(env(safe-area-inset-top) + 94px))}.checkpoint-pace-meter{display:block;position:relative;width:100%;height:3px;margin-top:5px;overflow:hidden;border-radius:99px;background:#ffffff17}.checkpoint-pace-meter::after{content:"";position:absolute;left:50%;top:0;width:1px;height:100%;background:#ffffff4d}.checkpoint-pace-meter b{display:block;width:50%;height:100%;border-radius:inherit;background:#bdefff;transform:scaleX(var(--pace-meter));transform-origin:right center;transition:transform .18s ease,background .18s ease}.checkpoint-pace-chip.ahead .checkpoint-pace-meter b{margin-left:0;background:#74f7c5;transform-origin:right center}.checkpoint-pace-chip.behind .checkpoint-pace-meter b{margin-left:50%;background:#ffd86b;transform-origin:left center}.checkpoint-pace-chip.best .checkpoint-pace-meter b{margin-left:0;width:100%;background:#8fffe0;transform:scaleX(1);transform-origin:center}.checkpoint-pace-chip.neutral .checkpoint-pace-meter b{margin-left:25%;width:50%;background:#bdefff;transform:scaleX(.08);transform-origin:center}@keyframes pace-recovered{0%{transform:translateX(-50%) scale(.96);filter:brightness(1)}45%{transform:translateX(-50%) scale(1.035);filter:brightness(1.2)}100%{transform:translateX(-50%) scale(1);filter:brightness(1)}}@media(max-height:390px){.checkpoint-pace-chip{top:max(78px,calc(env(safe-area-inset-top) + 66px));min-width:158px;font-size:8px;padding:4px 9px 6px}.checkpoint-pace-chip.boss{top:max(84px,calc(env(safe-area-inset-top) + 72px))}.checkpoint-pace-meter{margin-top:4px}}@media(prefers-reduced-motion:reduce){.checkpoint-pace-meter b{transition:none}.checkpoint-pace-chip.recovered{animation:none}}';
document.head.appendChild(paceStyle);
let latestPace=null,previousPaceState=null,recoveryTimer=0;
function clearRecovery(){if(recoveryTimer){clearTimeout(recoveryTimer);recoveryTimer=0;}paceChip.classList.remove('recovered');}
function clearPace(){latestPace=null;previousPaceState=null;clearRecovery();paceChip.hidden=true;paceChip.classList.remove('ahead','behind','best','neutral','boss');paceChip.style.setProperty('--pace-meter','0');paceChip.removeAttribute('aria-label');if(paceText)paceText.textContent='';}
function renderPace(detail){
 ...
 if(paceText)paceText.textContent=recovered?`PACE RECOVERED · ${stateText}`:`PACE · ${stateText}`;
 ...
 if(recovered){
  ...
  paceChip.classList.add('recovered');
  recoveryTimer=setTimeout(()=>{paceChip.classList.remove('recovered');recoveryTimer=0;},520);
 }
 paceChip.setAttribute('aria-label',recovered?(isBest?`${label}. Pace recovered with a new best split.`:`${label}. Pace recovered. ${Math.abs(delta).toFixed(1)} seconds ahead of best split.`):(isBest?`${label}. New best split.`:`${label}. ${Math.abs(delta).toFixed(1)} seconds ${delta<0?'ahead of':'behind'} best split.`));

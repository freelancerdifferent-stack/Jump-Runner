...
const boss={active:false,dead:false,hp:5,maxHp:5,x:7040,y:238,t:0,shot:0,hitCd:0,intro:0,flash:0,victory:0};
let bossShots=[];
...
function updateBoss(dt){
 ...
 if(boss.intro<=0){boss.shot-=dt;if(boss.shot<=0){boss.shot=Math.max(.55,1.25-(boss.maxHp-boss.hp)*.12);const sx=boss.x-30,sy=boss.y,dx=player.x+player.w/2-sx,dy=player.y+player.h/2-sy,l=Math.hypot(dx,dy)||1;bossShots.push({x:sx,y:sy,vx:dx/l*420,vy:dy/l*420,life:3.2,maxLife:3.2});}}
...
function drawBoss(){
 ...
 ctx.save();
 if(!boss.dead){const x=boss.x-cam,y=boss.y;ctx.translate(x,y);ctx.rotate(Math.sin(boss.t*3)*.08);ctx.shadowBlur=boss.flash>0?34:24;ctx.shadowColor=boss.flash>0?'#ffffff':'#ff6d88';ctx.fillStyle=boss.flash>0?'#fff3d4':'#1a2036';ctx.beginPath();ctx.arc(0,0,34,0,Math.PI*2);ctx.fill();ctx.strokeStyle='#ffd86b';ctx.lineWidth=5;ctx.beginPath();ctx.arc(0,0,25,boss.t,boss.t+Math.PI*1.45);ctx.stroke();ctx.fillStyle='#ff6d88';ctx.beginPath();ctx.arc(0,0,9,0,Math.PI*2);ctx.fill();ctx.setTransform(1,0,0,1,0,0);const bw=240,bx=VW/2-bw/2;ctx.globalAlpha=.95;ctx.fillStyle='#09111ecc';ctx.fillRect(bx,54,bw,24);ctx.fillStyle='#2c2031';ctx.fillRect(bx+4,58,bw-8,7);ctx.fillStyle='#ff6d88';ctx.fillRect(bx+4,58,(bw-8)*(boss.hp/boss.maxHp),7);ctx.fillStyle='#fff';ctx.font='800 10px system-ui';ctx.textAlign='center';ctx.fillText('SKY SENTINEL · '+boss.hp+'/'+boss.maxHp,VW/2,74);}

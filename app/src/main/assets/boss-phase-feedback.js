...
(()=>{
  const toast=document.createElement('div');toast.className='boss-phase-toast';toast.setAttribute('role','status');toast.setAttribute('aria-live','polite');toast.setAttribute('aria-atomic','true');document.body.appendChild(toast);
  ...
  function show(text,final){toast.textContent='';requestAnimationFrame(()=>{toast.textContent=text;toast.classList.toggle('final',!!final);toast.classList.remove('show');void toast.offsetWidth;toast.classList.add('show');timer=1.25;});}
  const base=update;
  update=function(dt){
    base(dt);
  ...
  const reset=resetRun;resetRun=function(){seen=0;timer=0;lastHp=boss.maxHp;toast.textContent='';toast.classList.remove('show');reset();};
})();

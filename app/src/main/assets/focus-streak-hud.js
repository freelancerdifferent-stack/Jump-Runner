'use strict';
// Replay clarity: keep the active coached-focus streak visible and show the next mastery target.
(()=>{
 ...
 function refresh(){const el=ensure(),value=read(),active=state==='play'&&!paused&&value>0;if(!active){el.hidden=true;return;}const mark=milestone(value),next=nextMilestone(value);el.hidden=false;el.className='focus-streak-hud'+(mark?' is-milestone':'')+(next?'':' is-complete');const lead=mark?`${mark.label} · FOCUS ×${value}`:`FOCUS ×${value}`;const remaining=next?next.value-value:0;el.innerHTML=next?`${lead}<span class="focus-next">${remaining} TO ${next.label}</span>`:lead;}

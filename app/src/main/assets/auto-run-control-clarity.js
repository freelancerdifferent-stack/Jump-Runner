...
(()=>{
  ...
  function hide(){clearTimeout(timer);timer=0;cue.classList.remove('show');cue.setAttribute('aria-hidden','true')}
  function show(){if(shown||typeof state==='undefined'||state!=='play')return;shown=true;hide();cue.classList.remove('show');void cue.offsetWidth;cue.setAttribute('aria-hidden','false');cue.classList.add('show');timer=setTimeout(hide,980)}
  function reset(){shown=false;hide()}

  // Control guidance should only appear once controls are actually live. Both countdown
  // layers expose explicit completion events, so avoid advertising Jump/Dash while locked.
  addEventListener('jumprunnercountdowncomplete',show);
  addEventListener('jumprunnerresumeready',show);
